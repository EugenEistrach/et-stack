...
const sheetVariants = cva('fixed z-50 bg-background shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out h-full flex flex-col', {
  variants: {
    side: {
...
const SheetHeader = ({
  className,
  ...props
}: React.HTMLAttributes<HTMLDivElement>) => <div className={cn('flex shrink-0 flex-col space-y-2 border-b bg-background px-6 py-4', className)} {...props} />;
SheetHeader.displayName = 'SheetHeader';
const SheetFooter = ({
  className,
  ...props
}: React.HTMLAttributes<HTMLDivElement>) => <div className={cn('flex shrink-0 flex-col-reverse border-t bg-background px-6 py-4 sm:flex-row sm:justify-end sm:space-x-2', className)} {...props} />;
